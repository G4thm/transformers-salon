    ...
    <PageShell>
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="mb-10 flex max-w-3xl flex-col gap-5">
          <Badge>Gallery</Badge>
          <h1 className="text-4xl font-black sm:text-5xl">Fox-led brand and salon gallery.</h1>
          ...
          {content.gallery.map((item) => (
            <figure key={item.id} className="glass mb-5 break-inside-avoid overflow-hidden rounded-lg">
              <div className="relative aspect-[4/3]">
                <Image src={item.image} alt={item.title} fill sizes="(min-width: 1024px) 33vw, 100vw" className="object-cover" loading="lazy" />
              </div>
